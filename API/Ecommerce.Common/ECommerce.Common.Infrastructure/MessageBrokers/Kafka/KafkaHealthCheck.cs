...
public class KafkaHealthCheck : IHealthCheck
{
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default(CancellationToken))
    {
        try
        {
            ...
            return result.Status == PersistenceStatus.NotPersisted ? new HealthCheckResult(context.Registration.FailureStatus, "Message was never transmitted to the broker, or failed with an error indicating it was not written to the log. Application retry risks ordering, but not duplication") : HealthCheckResult.Healthy();
        }
