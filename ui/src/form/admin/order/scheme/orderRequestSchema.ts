...
export const orderRequestSchema = Yup.object<IOrderRequestBody>().shape({
  ...
  discountValue: Yup.number()
    ...
    .when('discountType', (discountType: any) => (discountType === DiscountType.Percent ? Yup.number().max(100, 'Giá trị phần trăm phải nhỏ hơn hoặc bằng 100') : Yup.number())),
  discountPercent: Yup.number().required(),
  ...
  items: Yup.array().min(1, 'Vui lòng chọn ít nhất 1 sản phẩm').required('Vui lòng chọn ít nhất 1 sản phẩm cho đơn hàng').of(productItemSchema)
})
